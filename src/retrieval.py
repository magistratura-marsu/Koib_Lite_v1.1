...
from config import QUERY_PREFIX, VECTOR_SEARCH_K, BM25_SEARCH_K, FINAL_TOP_K, HYBRID_ALPHA, USE_RERANKER, RERANKER_MODEL, USE_ONNX_RERANKER, USE_HYDE, EMBEDDING_PROVIDER, METADATA_DIR, SEMANTIC_CACHE_ENABLED, SEMANTIC_CACHE_THRESHOLD, USE_USHAPED_CONTEXT, MAX_TABLE_ROWS_IN_PROMPT, MAX_TABLE_TOKENS_IN_PROMPT
...
class SemanticCache:
    ...
    def get(self, query: str, query_embedding: Optional[List[float]]) -> Optional[Dict]:
        ...
        try:
            ...
            cur.execute('SELECT query_text, embedding, answer, sources, query_hash FROM cache ORDER BY hit_count DESC, created_at DESC LIMIT 1000')
    ...
    def set(self, query: str, query_embedding: Optional[List[float]], answer: str, sources: List[Dict]) -> None:
        ...
        try:
            ...
            with self.conn: self.conn.execute('INSERT OR REPLACE INTO cache VALUES (?, ?, ?, ?, ?, CURRENT_TIMESTAMP, 0)', (q_hash, query, emb_blob, answer, json.dumps(sources, ensure_ascii=False)))
        except Exception: pass
...
class ResponseCache:
    ...
    def set(self, query: str, hypothetical: str) -> None:
        with self.conn: self.conn.execute('INSERT OR REPLACE INTO cache VALUES (?, ?)', (hashlib.md5(query.lower().strip().encode()).hexdigest(), hypothetical))
...
class HybridRetriever:
        ...
        if self.index_builder.text_vectorstore:
            try:
                ...
                for doc, score in docs:
                    ...
                    results.append(RetrievalResult(chunk_id=cid, content=doc.page_content, score=float(score), source=doc.metadata.get("source", ""), page=doc.metadata.get("page", 0), heading=doc.metadata.get("heading", ""), model=doc.metadata.get("model", "unknown"), chunk_type="text", metadata=doc.metadata))
            except Exception: pass
        if self.index_builder.summary_vectorstore:
            try:
                ...
                for doc, score in docs:
                    ...
                    results.append(RetrievalResult(chunk_id=cid, content=doc.page_content, score=float(score), source=doc.metadata.get("source", ""), page=doc.metadata.get("page", 0), heading=doc.metadata.get("heading", ""), model=doc.metadata.get("model", "unknown"), chunk_type=ct, metadata=doc.metadata))
            except Exception: pass
        return results
    ...
    def _bm25_search(self, query: str, model_filter: str) -> List[RetrievalResult]:
        ...
        for metadata, score in self.index_builder.bm25.search(query, k=BM25_SEARCH_K):
            ...
            results.append(RetrievalResult(chunk_id=metadata.get("chunk_id", ""), content=metadata.get("content", ""), score=score, source=metadata.get("source", ""), page=metadata.get("page", 0), heading=metadata.get("heading", ""), model=metadata.get("model", "unknown"), chunk_type=metadata.get("chunk_type", "text"), metadata=metadata))
        return results
    ...
    def _get_reranker(self):
        ...
        try:
            ...
            self._reranker = CrossEncoder(RERANKER_MODEL, backend="onnx" if USE_ONNX_RERANKER else None)
