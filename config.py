# -*- coding: utf-8 -*-
"""
Koib-V-4.5 — Конфигурация системы (оптимизировано для 2 ГБ RAM)
"""
import os
...
# ★ ИЗМЕНЕНО: VECTOR_SEARCH_K = 12 (компромисс скорость/recall)
VECTOR_SEARCH_K = int(os.getenv("VECTOR_SEARCH_K", "12"))
...
# ★ ИЗМЕНЕНО: русский реранкер (~100 МБ вместо 560 МБ)
RERANKER_MODEL = os.getenv("RERANKER_MODEL", "DiTy/ru-reranker-base")

# ★ ИЗМЕНЕНО: HyDE выключен (шумит на слабой LLM), используем query expansion
USE_HYDE = os.getenv("USE_HYDE", "false").lower() == "true"

# ★ НОВОЕ: стоп-слова BM25 (используется в indexing.py)
BM25_USE_STOPWORDS = os.getenv("BM25_USE_STOPWORDS", "true").lower() == "true"

# ★ ИЗМЕНЕНО: MAX_TOKENS = 1024 (700 резало таблицы)
